...
JsSIP.c = {
  ...
  // SIP Attributes
  MAX_FORWARDS: 69,
  ALLOWED_METHODS: 'INVITE, INFO, ACK, CANCEL, BYE, OPTIONS, MESSAGE, SUBSCRIBE',
...
JsSIP.Session.prototype.sendInfo = function(ct, body) {
  var
    extraHeaders = (ct) ? ['Content-Type: '+ ct] : [],
    request = this.dialog.createRequest(JsSIP.c.INFO, extraHeaders, body);
  this.sendRequest(request);
};
...
JsSIP.MediaSession.prototype = {
  ...
  */
  start: function(onSuccess) {
    ...
    this.peerConnection = new webkitRTCPeerConnection({"iceServers": servers}, pc_constraints);

    this.peerConnection.ongatheringchange = function(event) {
	if(!sent && event.currentTarget.iceGatheringState === "complete"){
	    sent = true;
	    onSuccess();
	}
    }
    ...
    this.peerConnection.onicecandidate = function(event) {
	  ...
	  if (0 && !sent && event.candidate.candidate.indexOf("srflx") > 0) {
	      sent = true;
	      console.log("WTF ???\n");
    ...
    this.peerConnection.onaddstream = function(mediaStreamEvent) {
	  ...
	  test = this.remoteStreams.length;
      } else {
	  test = this.getRemoteStreams().length;
      }

      if (session.remoteView && test > 0) {
        session.remoteView.src = webkitURL.createObjectURL(mediaStreamEvent.stream);
	  //console.log(session);


	//var local_audio_track = mediaStreamEvent.stream.getAudioTracks()[0];
	  //var dtmfSender = this.createDTMFSender(local_audio_track);
...
JsSIP.UA.prototype.hupall = function() {
...
JsSIP.UA.prototype.sendDTMF = function(ct, body) {
    var x = 0;

  for(session in this.sessions) {
      console.log(JsSIP.c.LOG_UA +'Sending: ' + body + ' to session' + session);
      this.sessions[session].sendInfo(ct, body); 
      x++;
  }
